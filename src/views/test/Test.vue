...
const filtered = computed(() => {
  if (!search.value) return items.value.map((item) => ({ value: item, title: item }));

  const f = items.value.filter((i) => i.toLowerCase().includes(search.value.toLowerCase()));

  // nếu không tìm thấy, trả về một fake item để hiển thị dòng Add
  return f.length
    ? f.map((item) => ({ value: item, title: item }))
...
const rows = [
  {
    label: "Adult",
    fn: (r) =>
      r?.worship_sessions?.reduce(
        (acc, session) => acc + (session.attendance.adult_attendance || 0),
        0,
      ),
  },
  {
    ...
    fn: (r) =>
      r?.worship_sessions?.reduce(
        (acc, session) => acc + (session.attendance.youth_attendance || 0),
        0,
      ),
  },
  {
    ...
    fn: (r) =>
      r?.worship_sessions?.reduce(
        (acc, session) => acc + (session.attendance.child_attendance || 0),
        0,
      ),
  },
  {
    ...
    fn: (r) =>
      r?.worship_sessions?.reduce(
        (acc, session) => acc + (session.attendance.total_attendance || 0),
        0,
      ),
...
<template>
  <pre>{{ val }}</pre>
  <!-- <NumberFormattedInput v-model="val" /> -->
  <OverseerChurchPlantedChartWidget :data="chartData" />

  <!-- <pre>{{ serviceModel }}</pre>
  <WorshipServiceInput v-model="serviceModel" :services="services" /> -->
